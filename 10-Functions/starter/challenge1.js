...
const poll = {
  ...
  registerNewAnswer() {
    const pollString = this.question + '\n' + this.options.join('\n');
    const response = prompt(pollString, 'Enter a number');
    if (this.validateAnswer(response)) {
      this.answers[response] += 1;
    } else {
      console.error('Response provided is not a valid answer');
    }
  ...
  validateAnswer(response) {
    ...
    return answer >= 0 && answer <= this.answers.length;
  },
